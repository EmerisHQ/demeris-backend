...
func TestMintInflation(t *testing.T) {
	...
	for _, ch := range testCtx.chains {
		t.Run(ch.Name, func(t *testing.T) {
			...
			// arrange
			url := fmt.Sprintf(baseUrl+mintInflationEndpoint, testCtx.emIngress.Protocol, testCtx.emIngress.Host, testCtx.emIngress.APIServerPath, ch.Name)
			// act
